...
**Status:** FROZEN v1.1 — supersedes v0.5. Repositioned to first-principles product framing: Build vs. Buy / incumbent-comparison content removed by deliberate decision, not oversight. Decision Strategy DSL section (§6) added; Replay-as-engineering-tool (§11) expanded. This document defines the "why." Further changes should be rare and driven by major shifts in business direction, not incremental polishing. Next: Product Roadmap.
...
**Runtime Engine** — deterministic execution: graph execution, rules, models, decision composition, audit, explainability, replay, simulation.

**Decision Studio** — where strategy engineers spend their time. Visual drag-and-drop components, a textual DSL, custom code extensions via a **Strategy SDK** — the same relationship an IDE has to its plugin ecosystem. Every node exposes configuration, validation, governance metadata, execution characteristics. Includes the **Decision Inspector** — the tool for stepping through a single decision's execution path node by node, the equivalent of a debugger for a strategy run. Feels like an engineering environment, not a workflow diagramming tool.

**Strategy Replay & Analysis Lab** — expected flagship capability. Historical datasets replayed through one or many strategies, producing high-level summaries and decision-level detail. Strategies compared side-by-side against the same historical population. Objective: understand the consequences of a strategy change *before* it reaches production, not just replay history for its own sake.

**Governance & Operations** — version control, strategy lifecycle, approval workflows, audit, replay, evidence packs, monitoring, observability. Everything required to operate the platform safely inside a regulated organisation.
...
AI is an assistant: document extraction, strategy explanation, migration assistance, DSL generation, documentation generation, engineering assistance. AI does not approve or decline applications. The platform always retains deterministic authority over customer decisions.
